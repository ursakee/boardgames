...
import { PlusCircle, Info, X, ArrowLeft } from "lucide-react";

const UnoLobbyPage: React.FC = () => {
  const navigate = useNavigate();
  const createGame = useGameStore((state) => state.createGame);
  const gameInfo = findGame("uno");
    ...
    <>
      <div className="relative w-full max-w-md p-8 text-center bg-slate-800 rounded-2xl shadow-2xl shadow-slate-950/50 border border-slate-700">
        ...
        <div className="flex flex-col items-center gap-4">
          <div className="p-3 bg-slate-700/50 rounded-full flex gap-1">
            <div className="w-8 h-8 bg-red-500 rounded-full text-white flex items-center justify-center font-bold text-xl">
              U
            </div>
            <div className="w-8 h-8 bg-yellow-400 rounded-full text-white flex items-center justify-center font-bold text-xl">
              N
            </div>
            <div className="w-8 h-8 bg-green-500 rounded-full text-white flex items-center justify-center font-bold text-xl">
              O
            </div>
          </div>
          <h2 className="text-4xl font-extrabold text-white">{gameInfo.displayName}</h2>
          <p className="text-slate-400 max-w-sm mx-auto">{gameInfo.description}</p>
