...
interface PreGameLobbyLayoutProps {
  ...
  players: Player[];
  isHost: boolean;
  gamePhase: "lobby" | "post-game";
  peerConnectionStates: Map<string, ConnectionState>;
  onLeaveGame: () => void;
  onSetUsername: (name: string) => void;
  onStartGame?: () => void;
  onPlayAgain?: () => void;
  children?: React.ReactNode; // For any game-specific controls
}

const PreGameLobbyLayout: React.FC<PreGameLobbyLayoutProps> = ({
  gameName,
  username,
  players,
  isHost,
  ...
  const areAllPlayersConnected = () => {
    ...
    // For a guest, just check the connection to the host ('p1')
    if (!isHost) {
      return peerConnectionStates.get("p1") === "connected";
    }

    // For the host, check connections to all other players
    const guestIds = players.filter((p) => p.id !== "p1").map((p) => p.id);
  ...
  const handleUpdateUsername = () => {
    if (currentUsername.trim()) {
      onSetUsername(currentUsername.trim());
    }
  };
  ...
  return (
    <div className="w-full max-w-lg p-8 space-y-6 bg-slate-800 rounded-2xl shadow-2xl shadow-slate-950/50 border border-slate-700">
      <h2 className="text-3xl font-bold text-center text-white capitalize">
        {gamePhase === "post-game" ? "Game Over" : `${gameName} Lobby`}
      </h2>
      ...
      {children && <div className="pt-4 border-t border-slate-700">{children}</div>}

      {/* Action Buttons */}
      <div className="pt-4 border-t border-slate-700 space-y-3">
        {isHost && gamePhase === "lobby" && onStartGame && (
          <button
            onClick={onStartGame}
        ...
        {isHost && gamePhase === "post-game" && onPlayAgain && (
          <button
            onClick={onPlayAgain}
...
export default PreGameLobbyLayout;
